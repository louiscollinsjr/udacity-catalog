...
def allTransportationItemJSON():
    ...
    return jsonify(transportationItems=[r.serialize for r in 
        transportationItems])
...
def searchLanding():
    ...
    if 'username' not in login_session:
        return render_template('search.html', globalMakers=data, makers=makers,
...
def showMakers():
    ...
    # return "This page will show all my makers"
    if 'username' not in login_session:
        return render_template('publicmakers.html', makers=makers,
            count_makers=count_makers )
    else:
        return render_template('makers.html', makers=makers,
            count_makers=count_makers, user=user)
...
def showVehicles():
    ...
    makers = session.query(Maker).all()
    # return "This page will show all my makers"
    return render_template('showAllVehicles.html', items=transportationItems,
        itemCount=itemCount)
...
def editMaker(maker_id):
    ...
    if login_session['user_id'] != item.user_id:
        return "<script>function myFunction() {alert('You are not authorized to modify this item.');}</script><body onload='myFunction()''>"
...
def showTransportation(maker_id):
    ...
    if 'username' not in login_session:
        return render_template('publictransportation.html', items=items, maker=maker)
    else:
        return render_template('transportation.html', items=items, maker=maker, user=user)
...
def newTransportationItem(maker_id):
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('showTransportation', maker_id=maker_id, user=user))
    else:
        return render_template('newtransportationitem.html', maker_id=maker_id, maker_name=maker.name, user=user)

    return render_template('newTransportationItem.html', maker=maker, creator=creator_id)
...
def editTransportationItem(maker_id, transportation_id):
    ...
    if request.method == 'POST':
        ...
        return render_template(
            'edittransportationitem.html', maker_id=maker_id, transportation_id=transportation_id, item=editedItem,user=user)

    # return 'This page is for editing transportation item %s' % transportation_id

# Delete a transportation item


@app.route('/maker/<int:maker_id>/transportation/<int:transportation_id>/delete',
           methods=['GET', 'POST'])
def deleteTransportationItem(maker_id, transportation_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('deleteTransportationItem.html', item=itemToDelete)
    # return "This page is for deleting transportation item %s" % transportation_id
...
def viewTransportationItem(maker_id, transportation_id):
    ...
    if 'username' in login_session:
        user = session.query(User).filter_by(email=login_session['email']).one()
    if 'username' not in login_session:
        return render_template('publicviewTransportationItem.html', maker_id=maker_id,transportation_id=transportation_id, item=item)
    else:
        return render_template('viewTransportationItem.html', maker_id=maker_id, transportation_id=transportation_id, item=item, user=user)
